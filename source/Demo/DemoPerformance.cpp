...
void DemoPerformance::InitializeSprite(Sprite& sprite)
{
    ...
    sprite.textureIndex = util::random(0, static_cast<int>(m_SpriteTextures.size()));
...
void DemoPerformance::ChangeSpriteCount(int new_count)
{
    ...
    {
        m_Sprites.resize(static_cast<size_t>(new_count));
...
void DemoPerformance::Draw() const
{
    ...
    for (int i = 0; i < m_SpriteCount; ++i)
    {
        ...
        const auto& texture = m_SpriteTextures[static_cast<size_t>(sprite.textureIndex)];
